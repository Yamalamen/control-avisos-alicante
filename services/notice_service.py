    ...
    *,
    ...
    if notify_username:
        ...
        body = (
            ...
            f"Numero de aviso: {current['notice_number']}.\n"
            f"Sede: {current.get('site') or 'Sin sede'}.\n"
            f"Estado: {normalized_status}.\n"
            f"Urgencia: {compute_urgency_color({**current, 'status': normalized_status})}.\n"
            f"Descripcion: {current.get('work_description') or 'Sin descripcion'}.\n"
            f"E.S.M.: {current.get('esm') or 'Sin E.S.M.'}.\n"
            f"Generador OT: {current.get('ot_generator') or 'Sin dato'}.\n"
            f"Generador Aviso: {current.get('notice_generator') or 'Sin dato'}.\n"
            f"Coordinador: {assigned_coordinator or 'Sin asignar'}.\n"
            f"Enlace Drive: {drive_url.strip() or 'Sin enlace'}.\n"
